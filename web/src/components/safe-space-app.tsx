      ...
      <main className="page-content admin-page">
        ...
        {tab === 'exams' && profile.data.name.trim().toLowerCase() === 'tyrese' && <section className="admin-panel">
          ...
          {exams.data?.length ? <div className="admin-list">{exams.data.map((exam) => <article className="admin-list-item csec-admin-submission" key={exam.match_id}>
            <div className="admin-list-item__top"><div><span className="status-badge">{exam.phase === 'complete' ? 'Submitted' : 'In progress'}</span><h3>{exam.player_name}</h3><small>{exam.submitted_at ? new Date(exam.submitted_at * 1000).toLocaleString() : 'Not submitted'} · {exam.time_spent_seconds == null ? 'Time not recorded' : `${Math.floor(exam.time_spent_seconds / 60)}m ${exam.time_spent_seconds % 60}s spent`}</small></div><div><strong>{exam.percentage.toFixed(1)}% · Paper 1: {exam.paper_one_scores[0] ?? 0}/{exam.paper_one.length} · Paper 2: {exam.paper_two_scores[0] ?? 0}</strong>{exam.phase === 'complete' && <button className="button button--secondary button--small" type="button" disabled={sendExamResults.isPending} onClick={() => sendExamResults.mutate(exam.match_id)}>{sendExamResults.isPending ? 'Sending…' : exam.results_email_sent_at ? 'Resend results email' : 'Send results email'}</button>}</div></div>
