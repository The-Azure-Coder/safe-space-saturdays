...
function QuotesScreen() {
  ...
  const queryClient = useQueryClient()
  const quotes = useQuery({ queryKey: ['quotes', category, 'slider'], queryFn: () => api.quotes(category, 1, 100) })
  ...
  const activeQuote = allQuotes[activeQuoteIndex] ?? allQuotes[0]
  const moveQuote = (direction: number) => {
    if (!allQuotes.length) return
    setActiveQuoteIndex((current) => (current + direction + allQuotes.length) % allQuotes.length)
  }
  useEffect(() => setActiveQuoteIndex(0), [category])
  useEffect(() => {
    if (activeQuoteIndex >= allQuotes.length && allQuotes.length) setActiveQuoteIndex(0)
  ...
  return <><PageHeader screen="quotes" /><main className="page-content quotes-page">{quotes.isLoading && <ContentSkeleton rows={3} />}<SectionHeading title="A little something for today" description="Words can uplift, comfort, and remind us we’re not alone. Take what you need, and come back anytime." /><article className="featured-quote" aria-roledescription="carousel" aria-label="Quote slider"><span className="quote-mark"><Quotes size={28} weight="fill" /></span><div key={activeQuote?.id ?? 'empty'} className="featured-quote__content"><blockquote>“{activeQuote?.text ?? 'Take a moment for yourself today.'}”</blockquote><cite>— {activeQuote?.author ?? 'Safe Space Saturdays'}</cite></div><div className="quote-slider-controls"><button type="button" className="quote-slider-arrow" aria-label="Previous quote" onClick={() => moveQuote(-1)} disabled={!allQuotes.length}><CaretLeft size={20} /></button><div className="carousel-dots" aria-label="Choose a quote">{allQuotes.map((quote, index) => <button type="button" className={index === activeQuoteIndex ? 'carousel-dot carousel-dot--active' : 'carousel-dot'} aria-label={`Show quote ${index + 1}`} aria-current={index === activeQuoteIndex ? 'true' : undefined} onClick={() => setActiveQuoteIndex(index)} key={quote.id} />)}</div><button type="button" className="quote-slider-arrow" aria-label="Next quote" onClick={() => moveQuote(1)} disabled={!allQuotes.length}><CaretRight size={20} /></button></div></article><div className="quote-actions"><button className="button button--primary" type="button" disabled={!activeQuote} onClick={() => activeQuote && save.mutate(activeQuote.id)}><BookmarkSimple size={20} /> {activeQuote?.saved ? 'Saved' : 'Save this quote'}</button><button className="button button--secondary" type="button" disabled={!activeQuote || share.isPending} onClick={() => activeQuote && share.mutate(`“${activeQuote.text}” — ${activeQuote.author}`)}><UsersThree size={20} /> {share.isPending ? 'Sharing…' : 'Share with community'}</button></div><section className="quote-share-card" aria-labelledby="write-quote-title"><div className="card-title"><PencilSimple size={22} /><span id="write-quote-title">Write your own</span></div><p>Share a few words that might give someone else a little hope today.</p><form onSubmit={(event) => { event.preventDefault(); const text = quoteDraft.trim(); if (text) share.mutate(text) }}><textarea value={quoteDraft} onChange={(event) => { setQuoteDraft(event.target.value); setShareStatus('') }} placeholder="Write an encouraging thought or personal quote…" maxLength={1000} aria-label="Write your own quote" /><div className="quote-share-card__footer"><small>{quoteDraft.length}/1000</small><button className="button button--primary button--small" type="submit" disabled={!quoteDraft.trim() || share.isPending}>{share.isPending ? 'Sharing…' : 'Share quote'}</button></div></form>{shareStatus && <p className="form-success" role="status">{shareStatus}</p>}{share.isError && <p className="form-error" role="alert">{share.error.message}</p>}</section><div className="filter-row">{['All', 'Encouragement', 'Rest', 'Growth', 'Connection'].map((filter, index) => <button className={category === filter ? 'filter-chip filter-chip--active' : 'filter-chip'} type="button" onClick={() => setCategory(filter)} key={filter}>{index === 0 ? <Leaf size={18} /> : index === 1 ? <Sparkle size={18} /> : index === 2 ? <span>☾</span> : <Leaf size={18} />}{filter}</button>)}</div></main><PageFooter /></>
}
