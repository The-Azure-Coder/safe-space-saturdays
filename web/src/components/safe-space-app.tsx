...
function Logo({ compact = false }: { compact?: boolean }) {
  return (
    <a className={`brand-mark ${compact ? 'brand-mark--compact' : ''}`} href="/" aria-label="Safe Space Saturdays home">
  ...
  return (
    <header className="app-header">
      ...
      {displayName ? <div className="profile-menu-wrap">
        ...
        {menuOpen && <div className="profile-dropdown" role="menu"><a href="/profile" role="menuitem" onClick={() => setMenuOpen(false)}>Profile & settings</a><button type="button" role="menuitem" onClick={() => logout.mutate()} disabled={logout.isPending}>{logout.isPending ? 'Signing out…' : 'Log out'}</button></div>}
      </div> : <div className="auth-actions" aria-label="Account actions"><a className="auth-actions__login" href="/login">Log in</a><a className="button button--primary button--small" href="/registration">Sign up</a></div>}
    </header>
  )
}
    ...
    <main className="auth-page">
      ...
      <section className="auth-panel" aria-labelledby="auth-title">
        ...
        <form onSubmit={onSubmit} noValidate>
          ...
          {isLogin ? <div className="form-row"><label className="checkbox-label"><input type="checkbox" defaultChecked /> <span>Remember me</span></label><a href="#forgot">Forgot password?</a></div> : <label className="checkbox-label"><input type="checkbox" required /> <span>I agree to the Safe Space Saturdays <a href="#privacy">Privacy Policy</a> and <a href="#terms">Terms of Service</a>.</span></label>}
        ...
        <p className="auth-switch">{isLogin ? 'New here?' : 'Already have an account?'} <a href={isLogin ? '/registration' : '/login'}>{isLogin ? 'Create an account' : 'Log in'}</a></p>
      </section>
    </main>
  )
}
  ...
  return <>
    ...
    <main className="page-content home-page">
      ...
      <section className="dashboard-grid">
        ...
        <article className="check-card"><div className="card-title"><Smiley size={22} weight="fill" /> <span>How are you feeling today?</span></div><p>Your check-in helps us support you better.</p><div className="mood-row">{moods.map((mood) => <button key={mood.label} type="button"><span>{mood.icon}</span><small>{mood.label}</small></button>)}</div><a className="button button--primary" href="/check-in">Check In</a></article>
        <article className="community-card"><div className="card-title"><UsersThree size={22} weight="fill" /> <span>Community Corner</span></div><h3>You are not alone.</h3><p>Join a space that listens, encourages, and grows together.</p><a className="button button--lilac" href="/community">Explore Community <ArrowRight size={16} /></a></article>
      </section>
      <GameStrip />
    </main>
    <PageFooter />
...
function GameStrip() {
  return <section className="game-strip"><div className="section-row"><div className="card-title"><GameController size={22} weight="fill" /> <span>Featured Games</span></div><a href="/games">View all games <ArrowRight size={16} /></a></div><div className="game-strip__items">{games.concat({ name: 'Bingo', players: '2+ players', icon: '🎯', color: 'peach' }).map((game) => <GameTile game={game} key={game.name} compact />)}</div></section>
}

function GameTile({ game, compact = false }: { game: (typeof games)[number]; compact?: boolean }) {
  return <article className={`game-tile game-tile--${game.color} ${compact ? 'game-tile--compact' : ''}`}><span className="game-tile__emoji" aria-hidden="true">{game.icon}</span><h3>{game.name}</h3><a className="button button--small button--primary" href="/games">Play</a>{!compact && <small>{game.players}</small>}</article>
}
...
function RangeRow({ label, left, right, accent = false, value, onChange }: { label: string; left: string; right: string; accent?: boolean; value?: number; onChange?: (value: number) => void }) {
  return <div className="range-row"><div><strong>{label}</strong><input aria-label={label} type="range" min="1" max="5" value={value ?? 3} onChange={(event) => onChange?.(Number(event.target.value))} /><span className="range-line"><i className={accent ? 'range-line__fill range-line__fill--orange' : 'range-line__fill'} /><i className="range-line__dot" /><i className="range-line__dot" /><i className="range-line__dot" /></span></div><div className="range-labels"><span>{left}</span><span>{right}</span></div></div>
}
...
function ProfileScreen() {
  ...
  if (profile.isError) return <><PageHeader screen="profile" /><main className="page-content profile-page"><SectionHeading eyebrow="Your space" title="Profile & settings" description="Sign in to manage your profile, privacy, and progress." /><section className="profile-card profile-auth-required"><ShieldCheck size={32} weight="fill" /><h2>Your profile is private</h2><p>Log in to view and update your account details.</p><a className="button button--primary" href="/login">Log in</a></section></main><PageFooter /></>
