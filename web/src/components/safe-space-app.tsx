...
function CommunityScreen() {
  ...
  const reply = useMutation({ mutationFn: ({ id, text }: { id: number; text: string }) => api.reply(id, text), onSuccess: (_, variables) => { setReplyDrafts((drafts) => ({ ...drafts, [variables.id]: '' })); queryClient.invalidateQueries({ queryKey: ['posts'] }); queryClient.invalidateQueries({ queryKey: ['replied-posts'] }) } })
