...
function AdminScreen() {
  ...
  const endExamSession = useMutation({
    mutationFn: api.endCsecExamSession,
    onSuccess: () => exams.refetch(),
  })
      ...
      <main className="page-content admin-page">
        ...
        {tab === 'exams' && staffRoles.has(profile.data.role) && <section className="admin-panel">
          ...
          {exams.data?.length ? <div className="admin-list">{exams.data.map((exam) => <article className="admin-list-item csec-admin-submission" key={exam.match_id}>
            <div className="admin-list-item__top"><div><span className="status-badge">{exam.phase === 'complete' ? (exam.status === 'completed' && !exam.results_email_sent_at ? 'Ended / submitted' : 'Submitted') : 'In progress'}</span><h3>{exam.player_name}</h3><small>{exam.submitted_at ? new Date(exam.submitted_at * 1000).toLocaleString() : 'Not submitted'} · {exam.time_spent_seconds == null ? 'Time not recorded' : `${Math.floor(exam.time_spent_seconds / 60)}m ${exam.time_spent_seconds % 60}s spent`}</small></div><div><strong>{exam.percentage.toFixed(1)}% · Paper 1: {exam.paper_one_scores[0] ?? 0}/{exam.paper_one.length} · Paper 2: {exam.paper_two_scores[0] ?? 0}</strong>{exam.phase !== 'complete' && <button className="button button--danger button--small" type="button" disabled={endExamSession.isPending} onClick={() => { if (window.confirm(`End ${exam.player_name}'s exam session? Their current answers will be saved.`)) endExamSession.mutate(exam.match_id) }}>{endExamSession.isPending ? 'Ending…' : 'End session'}</button>}{exam.phase === 'complete' && (() => { const graded = exam.grades[0]?.filter(Boolean).length ?? 0; const ready = graded === exam.paper_two.length; return <><small className="csec-admin-grade-progress">Paper 2 grading: {graded}/{exam.paper_two.length}</small><button className="button button--secondary button--small" type="button" disabled={sendExamResults.isPending || !ready} onClick={() => sendExamResults.mutate(exam.match_id)}>{sendExamResults.isPending ? 'Sending…' : exam.results_email_sent_at ? 'Resend results email' : ready ? 'Send results email' : 'Grade all questions first'}</button></> })()}</div></div>
            <div className="csec-admin-submission__answers"><h4>Paper 1 answers</h4>{exam.paper_one.map((question, index) => { const result = exam.paper_one_breakdown[index]; return <div className={result && result.points === 0 ? 'is-incorrect' : ''} key={index}><strong>{index + 1}.</strong> {question.question}<span>{exam.answers_one[0]?.[index] == null ? 'No answer' : question.options[exam.answers_one[0][index] as number]}</span></div> })}</div>
          ...
          {endExamSession.error && <p className="form-error" role="alert">{endExamSession.error instanceof Error ? endExamSession.error.message : 'The exam session could not be ended.'}</p>}
        </section>}
        {tab === 'applications' && (
