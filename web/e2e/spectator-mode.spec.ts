...
test('a member can open an active room with the eye action and watch read-only', async ({ page }) => {
  ...
  await page.route('**/api/games/sessions/match-1?*', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ match_id: 'match-1', room_id: 14, game: 'checkers', spectator: true, spectator_count: 2, state: { board: Array.from({ length: 8 }, () => Array(8).fill(0)), current_player: 0, winner: null, draw: false, players: [{ name: 'Host', color: 'coral', is_bot: false }, { name: 'Guest', color: 'sage', is_bot: false }], legal_moves: [], seat_index: -1, game_level: 2, game_streak: 1 } }) }))
  ...
  await expect(page.getByLabel('2 people watching')).toBeVisible()
